<?php
/*-----------------------------------------------------------------------------------*/	
/* Breadcrumb function| Updated on January 8, 2015
/*-----------------------------------------------------------------------------------*/
function iop_breadcrumb() {
global $post;
$parent_title = get_the_title( $post->post_parent );
	echo '<a href="';
	echo home_url();
    echo '">';
	echo __( 'Home','themonic');
	echo "</a>";
...
function themonic_pagination() {
	...
	$links = paginate_links( array(
		...
		'prev_text' => __( '&larr; Previous', 'themonic' ),
		'next_text' => __( 'Next &rarr;', 'themonic' ),
		'type'      => 'list',
	) );
	...
	<nav class="io-navigation" role="navigation">
		<h4 class="screen-reader-text"><?php _e( 'Posts navigation', 'themonic' ); ?></h4>
			<?php echo $links; ?>
	</nav><!-- .navigation -->
	<?php
	endif;
...
add_action( 'show_user_profile', 'iop_user_profile_fields' );
add_action( 'edit_user_profile', 'iop_user_profile_fields' );

function iop_user_profile_fields( $user ) { ?>
<h3><?php _e('Iconic One Pro User Profile Fields', 'themonic'); ?></h3>

<table class="form-table">
<tr>
<th><label for="linkedin"><?php _e('LinkedIn', 'themonic'); ?></label></th>
<td>
<input type="text" name="linkedin" id="linkedin" value="<?php echo esc_attr( get_the_author_meta( 'linkedin', $user->ID ) ); ?>" class="regular-text" /><br />
<span class="description"><?php _e('Please enter your LinkedIn ID.', 'themonic'); ?></span>
</td>
</tr>
<tr>
<th><label for="twitter"><?php _e('Twitter', 'themonic'); ?></label></th>
<td>
<input type="text" name="twitter" id="twitter" value="<?php echo esc_attr( get_the_author_meta( 'twitter', $user->ID ) ); ?>" class="regular-text" /><br />
<span class="description"><?php _e('Please enter your Twitter ID.', 'themonic'); ?></span>
</td>
</tr>
<tr>
<th><label for="facebook"><?php _e('Facebook', 'themonic'); ?></label></th>
<td>
<input type="text" name="facebook" id="facebook" value="<?php echo esc_attr( get_the_author_meta( 'facebook', $user->ID ) ); ?>" class="regular-text" /><br />
<span class="description"><?php _e('Please enter your Facebook ID.', 'themonic'); ?></span>
</td>
</tr>
<tr>
<th><label for="instagram"><?php _e('Instagram', 'themonic'); ?></label></th>
<td>
<input type="text" name="instagram" id="instagram" value="<?php echo esc_attr( get_the_author_meta( 'instagram', $user->ID ) ); ?>" class="regular-text" /><br />
<span class="description"><?php _e('Please enter your Instagram ID.', 'themonic'); ?></span>
</td>
</tr>
</table>
<?php }

add_action( 'personal_options_update', 'iop_save_profile_fields' );
add_action( 'edit_user_profile_update', 'iop_save_profile_fields' );

function iop_save_profile_fields( $user_id ) {
...
wp_enqueue_style('fontawesome-css', get_stylesheet_directory_uri().'/fonts/font-awesome.min.css' );
}
...
.login h1 a {background-size: auto; height: auto; width:320px; min-height: 75px;}
</style>';
}
}
...
add_filter( 'login_headerurl', 'iop_loginlogo_url' );

function iop_loginlogo_url($url) {
	return get_home_url();
}
/**
...
$options = get_option('iconiconepro');
if( isset ($options['themonic_slider'])&& $options['themonic_slider'] == '1' || $options['themonic_category_slider'] == '1'  ) {
...
function themonic_slider_css() {
    wp_enqueue_style('flexslider', get_stylesheet_directory_uri().'/js/flexslider.css');
}
add_action('wp_enqueue_scripts', 'themonic_slider_css');
/**flexslider and its css **/
}

/**Iconic One Pro custom function to Limit tags**/
add_filter('term_links-post_tag','ioptags');
function  ioptags($terms) {
	if (is_home() ) {return array_slice($terms,0,3,true);}
	else 	{return array_slice($terms,false);}
}
/**Limit tags**/
...
class Breadcrumb_Trail {
    ...
    protected function set_labels() {
        ...
        $defaults = array(
            'browse'              => esc_html__( 'Browse:',                               'themonic' ),
            'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'themonic' ),
            'home'                => esc_html__( 'Home',                                  'themonic' ),
            'error_404'           => esc_html__( '404 Not Found',                         'themonic' ),
            'archives'            => esc_html__( 'Archives',                              'themonic' ),
            // Translators: %s is the search query.
            'search'              => esc_html__( 'Search results for: %s',                'themonic' ),
            // Translators: %s is the page number.
            'paged'               => esc_html__( 'Page %s',                               'themonic' ),
            // Translators: %s is the page number.
            'paged_comments'      => esc_html__( 'Comment Page %s',                       'themonic' ),
            // Translators: Minute archive title. %s is the minute time format.
            'archive_minute'      => esc_html__( 'Minute %s',                             'themonic' ),
            // Translators: Weekly archive title. %s is the week date format.
            'archive_week'        => esc_html__( 'Week %s',                               'themonic' ),
    ...
    protected function add_minute_hour_archive_items() {
        ...
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'themonic' ) ) );
    }
    ...
    protected function add_minute_archive_items() {
        ...
        // Add the minute item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'themonic' ) ) );
    }
    ...
    protected function add_hour_archive_items() {
        ...
        // Add the hour item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'themonic' ) ) );
    }
    ...
    protected function add_day_archive_items() {
        ...
        $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'themonic' ) ) );
        $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'themonic' ) ) );
        $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'themonic' ) ) );
    ...
    protected function add_week_archive_items() {
        ...
        $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'themonic' ) ) );
        $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'themonic' ) ) );
    ...
    protected function map_rewrite_tags( $post_id, $path ) {
        ...
        if ( is_array( $matches ) ) {
            ...
            foreach ( $matches as $match ) {
                ...
                if ( '%year%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'themonic' ) ) ) );

                // If using the %monthnum% tag, add a link to the monthly archive.
                elseif ( '%monthnum%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'themonic' ) ) ) );

                // If using the %day% tag, add a link to the daily archive.
                elseif ( '%day%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'themonic' ) ) ) );
