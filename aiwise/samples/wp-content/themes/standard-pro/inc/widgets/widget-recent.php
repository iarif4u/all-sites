 ...
 * @since      1.0.0
 */
class standard_pro_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-standard_pro-recent widget_posts_thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'standard-pro' )
		);

		// Create the widget.
		parent::__construct(
			'standard_pro-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'standard-pro' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'standard_pro_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'standard_pro_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'standard_pro_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'standard-pro' ),
			'limit'     => 5,
			'show_date' => true
		);
